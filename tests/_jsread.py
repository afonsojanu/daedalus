...
def _class_body(mask, opening):
    boundary = max(
        mask.rfind(';', 0, opening),
        mask.rfind('{', 0, opening),
        mask.rfind('}', 0, opening),
    )
    return re.search(
        r'(?<![\w$])class(?:\s+[A-Za-z_$][\w$]*)?'
        r'(?:\s+extends\s+[^;{}]+)?\s*$',
        mask[boundary + 1:opening],
    ) is not None


def _object_body(mask, opening):
    previous = opening - 1
    while previous >= 0 and mask[previous].isspace():
        previous -= 1
    if previous < 0 or mask[previous] in '=([,:':
        return True
    word, _start = _word_before(mask, opening)
    return word in {'return', 'yield'}


def js_local_var_ranges(mask):
    """Return function and class-static ranges that locally scope `var`."""
    matching_open = _parenthesis_opens(mask)
    ...
    for opening, char in enumerate(mask):
        if char == '}':
            if not braces:
                continue
            start, kind = braces.pop()
            if kind in {'function', 'static'}:
                ranges.append((start, opening + 1))
            continue
        if char != '{':
            continue
        ...
        while previous >= 0 and mask[previous].isspace():
            previous -= 1
        parent_kind = braces[-1][1] if braces else None
        kind = 'block'
        if _class_body(mask, opening):
            kind = 'class'
        elif (parent_kind == 'class'
              and _word_before(mask, opening)[0] == 'static'):
            kind = 'static'
        elif mask[previous - 1:previous + 1] == '=>':
            kind = 'function'
        if previous in matching_open:
            ...
            if word not in controls or parent_kind in {'class', 'object'}:
                kind = 'function'
        elif _object_body(mask, opening):
            kind = 'object'
        braces.append((opening, kind))
    return ranges


def js_in_for_header(mask, start):
    """Whether `start` lies in an unmatched for/for-await header."""
...
def js_var_declaration_end(mask, start):
    ...
    in_for_header = js_in_for_header(mask, start)
